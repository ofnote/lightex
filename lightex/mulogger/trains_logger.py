...
class TrainsConfig:
    ...
    TRAINS_CONFIG_FILE: str ="./trains.conf" #generated by trains-init
    '''
    ...
    def __post_init__(self):
        tr_conf = Path(self.TRAINS_CONFIG_FILE).resolve()
        assert tr_conf.exists(), f"Trains config file {tr_conf} does not exist"
        self.TRAINS_CONFIG_FILE = tr_conf

    def get_env(self):
        return [('TRAINS_CONFIG_FILE', self.TRAINS_CONFIG_FILE)]
...
class TrainsLogger(AbstractLogger):
    ...
    def __init__(self, project_name=None, experiment_name=None, C: TrainsConfig=None):
        import trains
        self.trains = trains

        self.C = TrainsConfig() if C is None else C
